...
document.addEventListener("DOMContentLoaded", () => {
	...
	const admin = JSON.parse(localStorage.getItem('adminLogado'));// a const admin armazena as informações do adminLogado do local storage

	if (!admin) {// se não tiver nenhum admin no local storage, vai voltar para a página de login e não vai permitir o acesso
		window.location.href = 'login.html'
	} else {
		console.log('Usuário autenticado: ', admin.email);// se tiver um admin, exibe no console o email do admin
	}
